...
% Headers & Subheaders
% -------------------------
    \node[subtitle, text width=13em] (sub2) at (6, -0.5) {\texttt{CoroutineImpl::doResume}};
    \node[subtitle, text width=10em] (sub3) at (13.5, -0.5) {suspend fun};

% -------------------------
% Nodes - Column 2 (X=6) & Branches
% -------------------------
    \node[blueNote] (n2_1) at (6, -2.5) {\texttt{resume}};

    % Intermediate Branch Source (Centered under CoroutineImpl::doResume)
    \node[whiteNote, font=\scriptsize] (fork_src) at (6, -5.5) {execute\\suspendLambda};

    % Left Branch (X=4.5, centered around column)
    \node[whiteNote, font=\scriptsize, text width=7em] (fork_l) at (4.5, -7.5) {suspendLambda\\returned\\\texttt{COROUTINE\_SUSPENDED}};

    % Right Branch (X=7.5, centered around column)
    \node[whiteNote, font=\scriptsize, text width=7em] (fork_r) at (7.5, -7.5) {suspendLambda\\returned a value};

    \node[blueNote] (n2_2) at (6, -9.5) {\texttt{resume}};

% -------------------------
% Nodes - Column 3 (X=13.5)
% -------------------------
    \node[yellowNote] (n3_1) at (13.5, -2.5) {*execute\\suspend\\fun code*};

    % Container Box
    \node[container, minimum width=11em, minimum height=4.8em] (box) at (13.5, -5.5) {};
    ...
    \node[blueNote, font=\scriptsize, minimum height=2em, text width=7em] (n3_2) at (13.5, -5.8) {\texttt{suspend}};

    \node[yellowNote] (n3_3) at (13.5, -9.5) {*continue\\execution*};
    \node[yellowNote] (n3_4) at (13.5, -12) {return\\result};
