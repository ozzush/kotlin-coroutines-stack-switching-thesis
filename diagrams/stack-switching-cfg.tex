...
\node[block, below=of loop-end] (consumer-return) {Return};
...
\draw[arrow] (loop-back-gen) -- (generator-return);

% Stack-switching transfers
\draw[transfer] (create-cont) -- (generator-start);
\draw[arrow] (resume) -- node[above, pos=0.5] {first iteration} (generator-start);
