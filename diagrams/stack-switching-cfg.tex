...
\node[action, below=of resume] (handle-result) {Handle result};
...
\node[block, below=of loop-end] (consumer-return) {Return (done)};
...
\node[block, below=of suspend] (loop-back-gen) {Loop back};
...
\draw[arrow] (loop-end) to[out=0, in=0] (consumer-loop);
...
\draw[solidarrow] (resume) -- node[above, pos=0.3] {} (generator-start);
\draw[solidarrow] (resume) -- node[above, pos=0.3] {} (suspend);
\draw[solidarrow] (suspend) -- node[below, pos=0.7] {suspend (value + cont)} (handle-result);
\draw[solidarrow] (generator-end) -- node[below, pos=0.6] {return} (loop-end);
