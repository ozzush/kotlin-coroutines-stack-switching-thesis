...
% -------------------------
    ...
    \node[primitive] (n2) at (0, -3.5) {\texttt{resume}};

    % Use note style for description, correct typo "continuaion" -> "continuation"
    \node[note] (n3) at (0, -6) {*\textcolor{red}{continuation} is on\\the stack*};

    \node[primitive] (n4) at (0, -8.5) {\texttt{resume}};

    % Correct typo "reutrn" -> "return"
    \node[note] (n5) at (0, -13.5) {*return value is on\\the stack*};

% -------------------------
% Right Column Nodes (Suspend Function Context)
% -------------------------
    % Align this with the first resume (n2)
    \node[note] (n6) at (7, -3.5) {*execute\\fun body*};

    % Use primitive style for Wasm suspend instruction
    \node[primitive] (n7) at (7, -6) {\texttt{suspend}};

    % Note below n7
    \node[note] (n8) at (7, -8.5) {*resume value is on\\the stack*};

    % Note below n8
    \node[note] (n9) at (7, -11) {*execute\\fun body*};

    % Note below n9
    \node[note] (n10) at (7, -13.5) {return\\value};
