...
TODO(for each item specify which directory handles the corresponding tests)
\begin{itemize}
    \item \textbf{Control flow}: verifying that coroutines work correctly with \texttt{if}, \texttt{while}, \texttt{try-catch-finally}, and other control flow structures.
    \item \textbf{Default and variadic parameters}: ensuring that suspend functions with default or variadic arguments are handled correctly.
    \item \textbf{Exception handling}: verifying that exceptions thrown inside coroutines are correctly propagated to the completion continuation or handled by \texttt{try-catch} blocks.
\end{itemize}
